...
function parseUrls (urls, options, callback) {
    ...
    var start = new Date();
    logger.info('---------------------------------------------')
    logger.info('Elsewhere-mapper started - with list of url\'s');

    var urlArr = convertStringToArr(urls, 'urls');
    var results = parseDomainsFromUrls(urlArr);
    var ms = new Date().getTime() - start.getTime();
    logger.info('total time taken: ' + ms + 'ms');    
    if(callback){
        callback( null, results );
    }
    deferred.resolve( null, results );
}
 ...
 * callback: a function which is passes back mapper object  
 */
function parseSgns (sgns, options, callback) {
    var deferred = _.Deferred(),
        promise  = deferred.promise();
    ...
    var start = new Date();
    logger.info('---------------------------------------------')
    logger.info('Elsewhere-mapper started - with list of sgn\'s');

    var sgnsArr = convertStringToArr(sgns, 'sgns');
    var results = parseDomainsFromSgns(sgnsArr);
    var ms = new Date().getTime() - start.getTime();
    logger.info('total time taken: ' + ms + 'ms');
    if(callback){
        callback( null, results );
 ...
 * callback: a function which is passes back mapper object 
 */
function parseElsewhereJson (json, options, callback) {
    var deferred = _.Deferred(),
        promise  = deferred.promise(),
        err = null;
    ...
    var start = new Date();
    logger.info('---------------------------------------------')
    logger.info('Elsewhere-mapper started - with json data');

    var results = parseDomainsFromJson(json);
    var ms = new Date().getTime() - start.getTime();
    logger.info('total time taken: ' + ms + 'ms');
    if(callback){
    ...
    for (var x = 0; x < sgnArr.length; x++) {
        ...
        for (var i = 0; i < mapdata.length; i++) { 
            ...
            if(compareRootDomain(sgnDomain, mapdata[i].domain) ){
                ...
                // log the found sgn
                logger.info('mapping: ' + identity.sgn);
...
postProcess = function(items){
    ...
    if(notMapped.length > 0)
        out.domainsNotMapped = notMapped.sort();

    if(notParsed.length > 0)
        out.urlsNotParsed = notParsed.sort();

    // metadata
    username = commonUserName(identities);
    site = highestFrequencySite(identities, notMapped, false);

    if(commonUserName !== '')
        out.commonUserName = username;

    if(site !== '')
        out.highestFrequencySite = site;
...
function highestFrequencySite(identities, notMapped, noIdentities){
  ...
  // the frequency has to be higher then 1
  if(highest == 1){
    out = ''; 
  } 
...
// matched on lowercase
isUniqueIdentity = function(identityA, identityB) {
    return (identityA.sgn.toLowerCase() === identityB.sgn.toLowerCase());
}
